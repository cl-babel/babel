                 ...
                 (loop with nchars fixnum = 0
                    ...
                    while (< i end) do
                      (let* ((octet (,getter seq i))
                        ...
                        (declare (type ub8 octet) (fixnum next-i))
                        (cond ((> next-i end)
                               (decoding-error (vector octet) :johab seq i nil 'end-of-input-in-character)
                               (return (values (1+ nchars) end)))
                              (t
                               (setq nchars (1+ nchars)
                                     i next-i)
                               (when (and (plusp max) (= nchars max))
                                 (return (values nchars i))))))
                                   ...
                                   ;(format t "johab code-point-counter n-chars=~a / i=~a.~%" nchars i)
                                   (return (values nchars i))))))
        ...
        for x = (ucs->johab code) do
          (macrolet ((set-octet (offset value)
            ...
            (if (<= code #x80)
                ...
                (if (null x)
                    ;; unknown-code.
                    (error "skip?" :no-mapping-for-johab)
                    ;; ok.
                    (progn
                      (set-octet 0 (f-logand #xff (f-ash x -8)))
                      (set-octet 1 (logand x #xff))                       
